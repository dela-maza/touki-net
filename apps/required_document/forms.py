# apps/required_document/forms.py
...
class DocumentItemForm(WTForm):
    name   = StringField("書類名",     validators=[Optional(), Length(max=255)])
...
class RequiredDocumentForm(FlaskForm):
    ...
    entrusted_book_name = StringField("受託簿名（注書き可）", validators=[DataRequired(), Length(max=255)])
    ...
    mailed_documents = FieldList(FormField(DocumentItemForm), min_entries=5, max_entries=50, label="送付書類")
    requested_return_documents = FieldList(FormField(DocumentItemForm), min_entries=5, max_entries=50, label="返送依頼書類")
