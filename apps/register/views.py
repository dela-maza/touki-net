...
def upload():
    ...
    return render_template("register/upload.html", form=form, title="Register - Upload")
...
def _save_upload(file_storage) -> tuple[str, str]:
    """
    アップロードファイルを /uploads/register/ に保存して token を返す
    戻り値: (token, abs_path)
    """
    upload_root = current_app.config["UPLOAD_FOLDER"]
    save_dir = os.path.join(upload_root, "register")
    os.makedirs(save_dir, exist_ok=True)

    token = uuid.uuid4().hex  # 表示用トークン
...
def _find_saved_path(token: str) -> str | None:
    ...
    save_dir = os.path.join(current_app.config["UPLOAD_FOLDER"], "register")
...
def upload():
    ...
    if request.method == "POST":
        ...
        # 解析は別エンドポイントで実行（PRG）
        return redirect(url_for("register.parse", token=token), code=303)
...
def parse(token: str):
    ...
    if not pdf_path or not os.path.exists(pdf_path):
        ...
        return redirect(url_for("register.upload"))

    # --- ここで実パーサに差し替える想定 ---
    # 例）from apps.register.parser.commerce import CommerceRegisterParser
    ...
    return render_template("register/parse_result.html",
