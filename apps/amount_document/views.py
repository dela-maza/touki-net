...
def index() -> str:
    ...
    return render_template("amount_document/index.html", documents=documents, param=_param)
...
def detail(document_id: int) -> str:
    ...
    _param = load_config()["OFFICE"]
    document_type_label = {
        e.value: e.label for e in AmountDocumentType
...
def create() -> Union[str, WerkzeugResponse]:
    ...
    doc_type = AmountDocumentType.from_id(form.document_type.data or AmountDocumentType.ESTIMATE.value)
    form.note.data = get_note_default_by_document_type(doc_type)
